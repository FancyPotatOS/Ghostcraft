...
scoreboard players set survivors_left ghostcraft.temp 0
execute as @e[type=#ghostcraft:player,tag=ghostcraft.survivor] run scoreboard players add survivors_left ghostcraft.temp 1
execute if score survivors_left ghostcraft.temp matches 1 run tellraw @a[tag=ghostcraft.survivor] {"color":"red","bold":true,"text":"You are the last survivor."}
