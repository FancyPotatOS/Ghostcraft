...
execute if items entity @s weapon.mainhand fishing_rod[custom_data~{ghostcraft:{type:"grappling_hook"}}] if data entity @s SelectedItem.components."minecraft:custom_data".ghostcraft{type:"grappling_hook"}.bobber_data{OnGround:1b} run function ghostcraft:item/grappling_hook/launch
